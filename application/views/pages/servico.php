...
include_once '../connection/connectServico.php'; // Inclui o arquivo de conexão com o banco de dados
if (!isset($_SESSION['user_level']) || ($_SESSION['user_level'] != 1 && $_SESSION['user_level'] != 3)) { // Verifica se o usuário tem o nível de acesso necessário
    echo "<script>alert('Acesso não autorizado!'); window.location.href = '../pages/dashboard.php';</script>"; // Exibe um alerta e redireciona para o dashboard se o acesso não for autorizado
...
<body>
    <?php $currentPage = basename($_SERVER['PHP_SELF'], ".php") ?> <!-- Obtém o nome da página atual -->
    <?php include_once '../includes/navbar.php'; ?> <!-- Inclui a barra de navegação -->
    <?php include_once '../includes/modalRelatorio.php'; ?> <!-- Inclui o modal de relatório -->
    <?php include_once '../includes/modalRelatorioCompleto.php'; ?> <!-- Inclui o modal de relatório completo -->
    <?php include_once '../includes/modalCadastrarServico.php'; ?> <!-- Inclui o modal de cadastro de serviço -->
    ...
    <div class="container" id="materiais-table">
            ...
            <table id="materiais" class="table table-striped table-bordered"> <!-- Tabela com classes do Bootstrap e DataTables -->
                <thead>
                    <tr>
                        ...
                        <th>Produto</th>
                        <th>Quantidade</th>
                        <?php if ($_SESSION['user_level'] == 1): ?> <!-- Verifica se o usuário é administrador -->
                        <th>Ação</th>
                        <?php endif; ?>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($dadosDoBanco as $row): ?> <!-- Itera sobre os dados do banco de dados -->
                        <tr>
                            ...
                            <?php if ($_SESSION['user_level'] == 1): ?> <!-- Verifica se o usuário é administrador -->
                            <td>
                                <input type='checkbox' class='form-check-input' /> <!-- Checkbox para ações -->
                            </td>
                            <?php endif; ?>
                        </tr>
                    <?php endforeach; ?>
        ...
        <button onclick="abrirModalCadastrarServico()" class="btn btn-primary" id="novo-servico"><i class="bi bi-plus-circle-fill me-2"></i>Novo serviço</button> <!-- Botão para abrir modal de cadastro de serviço -->
        <?php if ($_SESSION['user_level'] == 1): ?> <!-- Verifica se o usuário é administrador -->
